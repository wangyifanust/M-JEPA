...
class Transformer(nn.Module):
    ...
    class Teacher(nn.Module):
        def __init__(self, parent):
            ...
            self.t_patch_size = parent.t_patch_size
    ...
    def forward_loss(self, student_latent, teacher_latent, target, mask, ids_restore, student, teacher, ema_decay=0.999):
        ...
        lambda2 = 0.0
        contrastive_loss = F.mse_loss(target, teacher_motion, reduction='none')
...
class Model(nn.Module):
    def __init__(self, dim_in=3, dim_feat=256, decoder_dim_feat=256,
        ...
        self.decoder_pred_recon_mlp = MLP(
            in_features=decoder_dim_feat,
            out_features=t_patch_size * patch_size * dim_in,
            hidden_features=256,
            drop=self.drop_rate  
        )
        self.decoder_pred_mlp = MLP(
            in_features=decoder_dim_feat,
            out_features=decoder_dim_feat,
            hidden_features=256,
            drop=self.drop_rate
        )
        self.teacher_mlp = MLP(
            in_features=decoder_dim_feat,
            out_features=decoder_dim_feat,
            hidden_features=256,
            drop=self.drop_rate
        ...
        self.instance_norm_predictor = InstanceNorm1d(decoder_dim_feat,affine=False)
        self.norm_context_teacher= nn.LayerNorm(dim_feat)
        self.norm_context_student= nn.LayerNorm(dim_feat)
        self.apply(self._init_weights)
    ...
    def generate_normalized_teacher_targets(self, teacher_latents):
        ...
        to the teacher latent outputs from each FFN layer as described in the contextualized target representations.
        """
        ...
        for latent in teacher_latents:
            ...
            # Transpose latent to [N, C, L] for InstanceNorm1d if needed
            latent = latent.permute(0, 2, 1)  # Convert to [N, C, L] format for InstanceNorm1d
            latent_in = self.instance_norm(latent) 
            # latent_in = latent
            latent_in = latent_in.permute(0, 2, 1)  # Convert back to [N, L, C] format
        ...
        Y = self.norm_context_teacher(Y_prime)  # Assuming `self.norm_context_teacher` is an instance of `nn.LayerNorm`
        return Y
        # return Y_prime

    def generate_normalized_student_targets(self, student_latents):
        """
        Applies instance normalization followed by averaging and layer normalization
        ...
        for latent in student_latents:
            # Handle different shapes of latent to ensure compatibility
            if latent.ndim != 3:
                raise ValueError(f"Expected latent to have 3 dimensions, but got {latent.ndim} dimensions.")
            latent = latent.permute(0, 2, 1)
            latent_in = self.instance_norm_predictor(latent) * sqrt(latent.size(-1))
            # latent_in = latent
            latent_in = latent_in.permute(0, 2, 1)
            normalized_outputs.append(latent_in)
        
        # Average normalized features across layers
        Y_prime = torch.mean(torch.stack(normalized_outputs), dim=0)  # Average across layers
        Y= self.norm_context_student(Y_prime)  # Assuming `self.norm` is an instance of `nn.LayerNorm`
        return Y
        # return Y_prime
    ...
    def forward_loss(self, student_latent, student_orginal_motion, teacher_latent, target, mask, ids_restore, ema_decay=0.999):
        ...
        student_latent = self.decoder_pred_mlp(student_latent)
        # teacher_latent = self.teacher_mlp(teacher_latent)
        ...
        return recon_loss2,recon_loss, contrastive_loss_original.mean()
    ...
    def forward(self, x, mask_ratio=0.80, motion_stride=1, motion_aware_tau=0.75, **kwargs):
        ...
        # print('teacher_latent', teacher_latent.shape)
        loss,loss1,loss2 = self.forward_loss(student_latent2,student_orginal_motion, teacher_latent, x_motion, mask, ids_restore)
...
class Encoder(nn.Module):
    ...
    def motion_aware_random_masking(self, x, x_orig, mask_ratio, tau):
        ...
        # print('x1', x.shape)
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
        # x_mask_tokens = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
        mask_tokens = self.mask_token.repeat(NM, TP * VP - x_masked.shape[1], 1)
    ...
    def random_masking(self, x, mask_ratio):
        ...
        ids_keep = ids_shuffle[:, :len_keep]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
        mask_tokens = self.mask_token.repeat(N, L - x_masked.shape[1], 1)
        x_ = torch.cat([x_masked, mask_tokens], dim=1)# no cls token
        x_ = torch.gather(x_, dim=1, index=ids_restore.unsqueeze(-1).repeat(1, 1, x_.shape[2]))
        x_masked = x_
    ...
    def forward_encoder(self,x, mask_ratio, motion_aware_tau):
        ...
        if self.is_teacher==False:
            ...
            x, mask, ids_restore, _ = self.random_masking(x, mask_ratio=0.0)
        # apply Transformer blocks
        ...
        if self.is_teacher:
            ...
            return x, latent, mask, ids_restore
        else:
            for idx, blk in enumerate(self.blocks):
                x = blk(x)

            x = self.norm(x)
            return x, x, mask, ids_restore

    def forward(self, x, mask_ratio=0.0, motion_aware_tau=0.0):
        if self.is_teacher:
            with torch.no_grad():
                return self.forward_encoder(x, mask_ratio=0.0, motion_aware_tau=0.0)
        else:
            return self.forward_encoder(x, mask_ratio, motion_aware_tau)
